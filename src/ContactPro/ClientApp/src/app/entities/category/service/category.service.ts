...
import { DATE_TIME_FORMAT_UTC } from "app/config/input.constants";
...
export class CategoryService {
    ...
    find(id: number): Observable<EntityResponseType> {
        return this.http
            .get<ICategory>(`${this.resourceUrl}/${id}`, {
                observe: "response",
            })
            .pipe(
                map((res: EntityResponseType) =>
    ...
    public convertDateFromClient(category: ICategory): ICategory {
        return Object.assign({}, category, {
            created: category.created?.isValid()
                ? category.created.format(DATE_TIME_FORMAT_UTC)
                : undefined,
        });
    }
